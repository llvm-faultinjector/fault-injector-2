 ...
 public:
  ...
  static void insertDetermineLogicForRawFaultInjection(Module &M, Function &F, Instruction *I, Instruction *T, std::mt19937& R) {
    ...
    CallInst *CI = CallInst::Create(
        M.getOrInsertFunction("__faultinject_xor_marker_caller",
    ...
    BinaryOperator *fi =
        BinaryOperator::CreateXor(I, val, "rfi", val->getNextNode());
  ...
  static void insertSelectFaultInjectionTarget(Module &M, Function &F,
                                               Instruction *I, Instruction *T) {
    std::vector<Value *> args(1);
    args[0] = I;
    ArrayRef<Value *> args_arr(args);

    CallInst *CI =
        CallInst::Create(getSelectFaultInjectionTargetFunc(M), args_arr, "", I->getNextNode());
...
#pragma region Fault Injection Functions
  ...
  static Constant *getSelectFaultInjectionTargetFunc(Module &M) {
    std::vector<Type *> func_param_types(1);
    func_param_types[0] = Type::getInt32Ty(M.getContext());
...
struct LLVMFaultInjectionPass : public ModulePass {
  ...
  bool runOnModule(Module &M) override {
    ...
    for (Module::iterator m_it = M.begin(); m_it != M.end();
      ...
      FaultInjectionInsertMachine::insertSelectFaultInjectionTarget(M, *m_it, selected, selector.getSelectedInsts()[0].first);
      break;
#endif
    }
